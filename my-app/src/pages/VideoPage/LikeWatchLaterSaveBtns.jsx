...
function LikeWatchLaterSaveBtns({ vv, vid }) {
  ...
  useEffect(() => {
    likedVideoList?.data
      .filter(
        (q) => q?.videoId === vv._id && q?.Viewer === CurrentUser?.result._id
      )
      .forEach((m) => setLikeBtn(true));

    watchLaterList?.data
      .filter(
        (q) => q?.videoId === vv._id && q?.Viewer === CurrentUser?.result._id
      )
      .forEach((m) => setSAveVideo(true));
  }, [CurrentUser?.result._id, likedVideoList?.data, vv._id, watchLaterList?.data]);
